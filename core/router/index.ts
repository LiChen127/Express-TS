...
export default class Router implements RouterPrototype {
   ...
   */
  // 路由参数中间件
  private params: { [key: string]: Function[] } = {};
  // 全局参数中间件
  private _params: (string | Function)[] = [];
  // 路由栈
  private stack: any[] = [];
  // 严格模式
  private strict: boolean = false;
  // 是否合并父路由参数
  private mergeParams: boolean = false;
  // 是否区分大小写
  ...
  param(
    ...
    if (name[0] === ':') {
      ...
      console.warn(
        'Invalid router param name, param name can not start with ":"',
      );
      // 帮助用户去除错误字符
      name = name.slice(1);
    }
    ...
    this._params.forEach((paramMiddleware) => {
      callback = (paramMiddleware as Function)(name, callback);
    })
   ...
   * @param next
   */
  private handle(req: Request, res: Response, out: NextFunction): void {
    // 中间件索引
    let index = 0;
    // 获取协议和主机
    ...
    function next(err?: any): void {
      ...
      while (!match && index < stack.length) {
         ...
         * 处理参数错误回调
         */
        const processParamErrorCallback = (err: any) => {
          if (err) {
            next(layerError || err);
          } else if (route) {
     ...
     * @param path 路径
     */
    function trimPrefix(layer: any, layerError: any, layerPath: string, path: string): void {
    ...
    function processParams(layer: any, paramCalled: Map<string, any>, req: Request, res: Response, done: Function): void {
   ...
   */
  private appendMethods(list: any[], methods: string[]): void {
    // list.push(method);
   ...
   */
  private sendOptonsResponse(res: Response, options: any[], done: Function): void {
    try {
   ...
   * @returns 
   */
  private mergeParamsFn(params: any[], parent: any): any[] {
    /**
   ...
   * @returns 
   */
  private wrap(old: Function, fn: (...args: any[]) => any) {
    /**
     * 代理
     */
